...
func (r *postgresTicketRepository) ListTickets(ctx context.Context, sellerID string) ([]*models.Ticket, error) {
	...
	for attempts := 0; attempts < 2; attempts++ {
		if sellerID != "" {
			rows, err = r.db.QueryContext(ctx, query+" WHERE t.seller_id = $1 ORDER BY t.created_at DESC", sellerID)
		} else {
			rows, err = r.db.QueryContext(ctx, query+" ORDER BY t.created_at DESC")
		}
